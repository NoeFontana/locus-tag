...
//! - `cargo run --bin stub_gen --no-default-features --features profiles`
        ...
        if current != new_content {
            eprintln!(
                "locus/locus.pyi is out of date.\nRun `cargo run --bin stub_gen \
                 --no-default-features --features profiles` (under `uv run`) and commit it."
            );
            std::process::exit(1);
        }
...
/// - The pyo3 enums carry `eq_int`, so they are int-comparable at runtime.
///   pyo3-stub-gen emits plain `enum.Enum`; rewrite to `enum.IntEnum` so
///   `int(TagFamily.X)` type-checks (the wrapper passes families as ints).
#[cfg(all(feature = "stub-gen", not(feature = "extension-module")))]
