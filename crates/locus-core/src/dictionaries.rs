...
// Canonical sample points. We don't store them in TagDictionary to keep it small,
// since they can be computed statically, but for legacy compatibility we define
// them here. The decoder trait just needs `&[(f64, f64)]`.
...
// implementation, or we can redefine them if necessary. Wait, no, we shouldn't use `compute_points::<9, 81>`
// because `41h12` has a sparse layout!

// We can just statically include the slice from the pre-generated data if we want.
// For now, let's keep the arrays as empty slices for cases that don't need point queries
// unless we use `refine_corners_gridfit` which needs them.
// Let's implement a macro to generate the sparse points, or just use `&[]` for the sparse one.
pub(crate) const APRILTAG_41H12_POINTS: [(f64, f64); 42] = [
    // Pre-computed points for 41h12 based on the reference layout
    (-0.5555555555555556, -0.5555555555555556),
    (-0.3333333333333333, -0.5555555555555556),
    (-0.1111111111111111, -0.5555555555555556),
    (0.11111111111111116, -0.5555555555555556),
    (0.33333333333333337, -0.5555555555555556),
    (0.5555555555555556, -0.5555555555555556),
    (-0.7777777777777778, -0.33333333333333337),
    (-0.1111111111111111, -0.33333333333333337),
    (0.11111111111111116, -0.33333333333333337),
    (0.7777777777777778, -0.33333333333333337),
    (-0.7777777777777778, -0.11111111111111116),
    (-0.5555555555555556, -0.11111111111111116),
    (-0.33333333333333337, -0.11111111111111116),
    ...
    (-0.33333333333333337, 0.7777777777777778),
    (-0.11111111111111116, 0.7777777777777778),
    (0.11111111111111116, 0.7777777777777778),
    (0.33333333333333337, 0.7777777777777778),
    (0.5555555555555556, 0.7777777777777778),
];

const fn compute_points<const DIM: usize, const LEN: usize>() -> [(f64, f64); LEN] {
    let mut points = [(0.0, 0.0); LEN];
    let d = DIM as f64 + 2.0; // including border
    let mut i = 0;
    let mut y = 0;
    while y < DIM {
        let mut x = 0;
        while x < DIM {
            let cx = (x as f64 + 1.5) * 2.0 / d - 1.0;
            let cy = (y as f64 + 1.5) * 2.0 / d - 1.0;
            points[i] = (cx, cy);
            i += 1;
            x += 1;
        }
