...
use locus_core::pose::{refine_poses_soa, CameraIntrinsics};
...
fn test_refine_poses_soa_interface() {
    ...
    batch.corners[0].x = -10.0; batch.corners[0].y = -10.0;
    batch.corners[1].x = 10.0;  batch.corners[1].y = -10.0;
    batch.corners[2].x = 10.0;  batch.corners[2].y = 10.0;
    batch.corners[3].x = -10.0; batch.corners[3].y = 10.0;
    
    let v = 1;
    let intrinsics = CameraIntrinsics::new(500.0, 500.0, 320.0, 240.0);
    let tag_size = 0.16;
    
    refine_poses_soa(&mut batch, v, &intrinsics, tag_size, None, PoseEstimationMode::Fast);
