//! End-to-end verification that the three shipped JSON profiles deserialize
...
use locus_core::config::{
    CornerRefinementMode, DecodeMode, DetectorConfig, QuadExtractionMode, SegmentationConnectivity,
};
...
fn assert_shared_defaults(cfg: &DetectorConfig) {
    ...
    assert_eq!(cfg.adaptive_threshold_constant, d.adaptive_threshold_constant);
    assert_eq!(
...
fn standard_profile_matches_former_builder() {
    ...
    assert_eq!(cfg.segmentation_connectivity, SegmentationConnectivity::Eight);
...
fn grid_profile_matches_former_builder() {
    ...
    assert_eq!(cfg.decode_mode, DecodeMode::Hard);
    assert_eq!(cfg.quad_extraction_mode, QuadExtractionMode::ContourRdp);
    assert_eq!(cfg.segmentation_connectivity, SegmentationConnectivity::Four);
