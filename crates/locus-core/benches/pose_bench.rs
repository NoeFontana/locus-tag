use divan::bench;
use locus_core::pose::{estimate_tag_pose, CameraIntrinsics};
...
fn bench_pose_estimation(bencher: divan::Bencher) {
    ...
    bencher.bench_local(move || {
        divan::black_box(estimate_tag_pose(divan::black_box(&intrinsics), divan::black_box(&corners), divan::black_box(tag_size)))
    });
}
