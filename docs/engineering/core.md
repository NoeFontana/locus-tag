...
*   **Configuration is nested for humans, flat for machines, authored in JSON, embedded at build.** Detector settings live in one place: the shipped JSON profiles (`crates/locus-core/profiles/*.json`). Rust keeps its flat `DetectorConfig` struct for hot-path access; a serde shim nests at the JSON boundary. Python users interact with a nested Pydantic `DetectorConfig`. The three shipped profiles are `include_str!`'d into the Rust crate and re-exposed to the Python wheel through the `_shipped_profile_json` FFI hook — defaults cannot disappear because a file was deleted. If the JSON and a Rust constant disagree, **the JSON is authoritative** (see `docs/engineering/profiles.md`).
