...
def run_visualization(scenario, types=("tags",), limit=None):
    ...
    for img_name in tqdm(img_names, desc=f"Processing {scenario}"):
        ...
        for det in detections:
            ...
            if min_dist < 20.0 and best_gt_idx != -1 and gt_tags[best_gt_idx].tag_id == det.id and best_gt_idx not in matched_gt_indices:
                ...
                for ordering in [det_corners, det_corners[::-1]]:
                    for rot in range(4):
                        ...
                        err = np.sqrt(np.mean(np.sum((gt_tags[best_gt_idx].corners - rotated) ** 2, axis=1)))
    ...
    if any(valid_mask):
        plt.plot(np.array(x_vals)[valid_mask], np.array(errors)[valid_mask], linestyle='-', color='#d62728', linewidth=1.5)
    plt.xlabel(x_label)
