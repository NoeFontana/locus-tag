...
def get_manifest(repo_id, subset):
    try:
        manifest_path = hf_hub_download(repo_id=repo_id, filename=f"{subset}/manifest.json", repo_type="dataset")
...
def main():
    ...
    if manifest:
        if "camera_intrinsics" in manifest:
            ...
            print(f"Loaded intrinsics: fx={intrinsics.fx}, fy={intrinsics.fy}, cx={intrinsics.cx}, cy={intrinsics.cy}")
        if "tag_specification" in manifest:
            tag_size = manifest["tag_specification"]["tag_size_m"]
            print(f"Loaded tag size: {tag_size}m")

    detector = locus.Detector(families=[locus.TagFamily.AprilTag36h11], refinement_mode=locus.CornerRefinementMode.Erf)
    ...
    for item in ds:
        ...
        rr.log("world/camera/gt_corners", rr.LineStrips2D(gt_corner_strips, colors=[[0, 255, 0]] * len(gt_corner_strips), radii=1.0))
        
        if positions is not None and rotations is not None and all(p is not None for p in positions):
            for tid, pos, rot in zip(tag_ids, positions, rotations, strict=False):
                rr.log(f"world/camera/gt_tag_{tid}", rr.Transform3D(
                    translation=pos,
                    rotation=rr.Quaternion(xyzw=rot)
                ))
                rr.log(f"world/camera/gt_tag_{tid}/axes", rr.Arrows3D(
                    vectors=[[0.05, 0, 0], [0, 0.05, 0], [0, 0, 0.05]],
                    colors=[[255, 0, 0], [0, 255, 0], [0, 0, 255]]
                ))

        # Locus Predictions
        batch = detector.detect(img_np, intrinsics=intrinsics, tag_size=tag_size, pose_estimation_mode=locus.PoseEstimationMode.Fast)
        ...
        if pred_corner_strips:
            rr.log("world/camera/pred_corners", rr.LineStrips2D(pred_corner_strips, colors=[[255, 0, 0]] * len(pred_corner_strips), radii=1.0))
        ...
        for i in range(len(batch)):
            ...
            if pose is not None:
                ...
                rr.log(f"world/camera/pred_tag_{tid}", rr.Transform3D(
                    translation=[tx, ty, tz],
                    rotation=rr.Quaternion(xyzw=[qx, qy, qz, qw])
                ))
                rr.log(f"world/camera/pred_tag_{tid}/axes", rr.Arrows3D(
                    vectors=[[0.05, 0, 0], [0, 0.05, 0], [0, 0, 0.05]],
        ...
        for i in range(len(batch)):
            ...
            if det_id in tag_ids:
                ...
                if positions is not None and positions[gt_idx] is not None and batch.poses[i] is not None:
