...
# Example: ├─ bench_noisy_scene                5.791 ms      │ 13.33 ms      │ 9.87 ms       │ 9.728 ms      │ 100     │ 100
...
def update_baseline(output_file: Path, new_results: Dict[str, Any]):
    ...
    # Simple merge: new results overwrite old ones
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python update_micro_baselines.py <divan_output_file> [baseline_json]")
        sys.exit(1)
        
    input_file = Path(sys.argv[1])
    baseline_file = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("docs/benchmarking/divan_baseline.json")
