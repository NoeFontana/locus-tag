...
def _match_detection_to_gt(
    batch: locus.DetectionBatch, gt_tag_id: int
) -> int | None:
...
def _compute_corner_diagnostics(
    ...
    for i in range(4):
        ...
        eigs = lb.corner_structure_tensor_eigenvalues(
            img, cx, cy, structure_tensor_radius
        )
        ...
        if (
            ...
            try:
                ...
                loo_drop_pct = float(
                    (rot_err_chosen - rot_err_loo_vs_gt) / rot_err_chosen * 100.0
                )
        ...
        rec = CornerRecord(
            ...
            final_mahalanobis_d2=(
                float(final_d2[i]) if not np.isnan(final_d2[i]) else None
            ),
            final_irls_weight=(
                float(final_w[i]) if not np.isnan(final_w[i]) else None
            ),
    ...
    summary = {
        ...
        "lm_convergence": (
            int(lm_telemetry["convergence"]) if lm_telemetry else None
        ),
    }
...
def run(
    ...
    *,
    ...
    rich_path = (
        Path("tests/data/hub_cache") / config_name / "rich_truth.json"
    )
    ...
    for img_name in tqdm(image_keys, desc="extract"):
        ...
        det_pose = (
            np.asarray(batch.poses[det_idx], dtype=np.float64)
        ...
        if diag["pose"] is not None:
            ...
            if det_pose is not None:
                ...
                quat_diff_deg = _quat_angle_deg(
                    tuple(bench_pose7[3:7]), tuple(det_pose[3:7])
                )
        ...
        scene = SceneRecord(
            ...
            detected_quaternion_xyzw=(
                tuple(float(v) for v in det_pose[3:7]) if det_pose is not None else None
            ),
            detected_translation_xyz=(
                tuple(float(v) for v in det_pose[:3]) if det_pose is not None else None
            ),
    ...
    print()
    print(f"  scenes detected:        {sum(1 for s in scene_records if s.detected)}/{len(scene_records)}")
    print(f"  production misses:      {skipped_no_detection}")
    print(f"  pose-equivalence violations (>1e-3 m / 0.01°): {pose_diff_violations}")
    print(f"  rotation p50/p95/p99:   ", end="")
    rot_errs = sorted(
        [s.rotation_error_chosen_deg for s in scene_records if s.rotation_error_chosen_deg is not None]
    )
    if rot_errs:
        n = len(rot_errs)
