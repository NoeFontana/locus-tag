...
def parse_gt(csv_path):
    ...
    with open(csv_path) as f:
        ...
        for row in reader:
            ...
            if img not in gt_map: gt_map[img] = {}
            if tid not in gt_map[img]: gt_map[img][tid] = [None]*4
...
def main():
    ...
    # Log to file for headless environments
    rr.init("locus_debug", spawn=False)
    rr.save("debug.rrd")
    ...
    for img_path in tqdm(images):
        ...
        for gt in gt_tags:
            if gt["id"] not in det_ids:
                corners = np.vstack([gt["corners"], gt["corners"][0]])
                rr.log(f"missed/tag_{gt['id']}", rr.LineStrips2D(corners, colors=[255, 0, 0], radii=1.0))
        ...
        for d in detections:
            ...
            if best_dist < 20.0 and best_gt_idx not in matched_gt and gt_tags[best_gt_idx]["id"] == d.id:
